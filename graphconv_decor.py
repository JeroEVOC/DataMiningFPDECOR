...
parser.add_argument('--model_name', default='graphconv_decor', type=str)
...
class GraphConvDecor(torch.nn.Module):
    def __init__(self, in_dim, hid_dim, n_classes, mlp_width):
        ...
        self.lin_rel1 = nn.Linear(in_dim, hid_dim)
    ...
    def forward(self, data):
        ...
        hid = torch.matmul(adj, x)
        hid = self.lin_rel1(hid)
        hid = hid + self.resid_weight * self.lin_root1(x)
        hid = self.dropout(hid)
        output = torch.matmul(adj, hid)
        output = self.lin_rel2(output)
        output = output + self.resid_weight * self.lin_root2(hid)      
...
def train(data, iter):
    ...
    model = GraphConvDecor(768, 64, 2, mlp_width).to(device)
